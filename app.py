...
def upload():
    ...
    return render_template(
        "table.html", table=table_html, titles=sleep_data.columns.values
    )
...
def predict():
    ...
    plt.scatter(
        sleep_data[X_columns[2]],
        sleep_data["Total Sleep Score"],
        color="#E8E8ED",
        alpha=0.5,
        s=100,
    ...
    plt.scatter(
        X.iloc[:, 2], y_pred, color="#0071e3", s=150, label="Your Prediction", zorder=5
    )  # Ensure prediction appears on top
